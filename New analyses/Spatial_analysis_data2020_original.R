...
distances <- read.csv("C:/Users/Cisco/Documents/TFM/data/caracolesplotposition.csv", sep = ";") 
...
alfonsodata<-  read.table("C:/Users/Cisco/Documents/TFM/data/Data_2020/2020_data_models_phenol_overlap_Seeds_per_fruit.csv", header=T, sep=",")
...
moran.test(beetle.v$visits,mat2listw(plots.dists.inv)) # I= 0.37
moran.test(beetle.v$visits, nb2listw(w5)) # vecinos, I= 0.49
moran.plot(beetle.v$visits,mat2listw(plots.dists.inv), main= " Spatial Autocorrelation Beetles across plots")


mc.beetles <- moran.mc(beetle.v$visits, mat2listw(plots.dists.inv), nsim=99) #in internet it is said that The Montercalo
# has more accurate results then the moran.test function, it is more sensitive than the moran test. p-value= 0.01
plot(mc.beetles)
...
moran.test(flies.v$visits,mat2listw(plots.dists.inv)) # I= 0.17
moran.test(flies.v$visits, nb2listw(w5)) # vecinos, I= 0.23
...
moran.test(but.v$visits,mat2listw(plots.dists.inv)) # I= 6.731810e-03, almost homogenius
moran.test(but.v$visits, nb2listw(w5)) # vecinos, I= 0.0339
moran.plot(but.v$visits,mat2listw(plots.dists.inv),  main= "Spatial autocorrelation butterflies across plots")
moran.mc(but.v$visits, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.1
...
moran.test(bee.v$visits,mat2listw(plots.dists.inv))# I = 0.058
moran.test(bee.v$visits, nb2listw(w5))# I= 0.076
...
moran.test(junto$visit,mat2listw(plots.dists.inv)) # I= 0.5749
moran.test(junto$visit, nb2listw(w5)) # vecinos, I= 0.6948
...
par(mfrow=c(2,3))
plot(bet.corr, main=  main= " Beetles distribution across plots",xlab="Distance (m)", ylab="Correlation")
...
mc.bema<- moran.mc(BEMA.p$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.01
plot(mc.bema)
...
moran.test(CHFU.fit$seeds,mat2listw(plots.dists.inv)) # I= 0.3973
moran.test(CHFU.fit$seeds, nb2listw(w5)) # vecinos, I= 0.4951
...
moran.test(LEMA.fit$seeds,mat2listw(plots.dists.inv)) # I= 0.289
moran.test(LEMA.fit$seeds, nb2listw(w5)) # vecinos, I= 0.3756
...
moran.test(MESU.fit$seeds,mat2listw(plots.dists.inv)) # 0.54
moran.test(MESU.fit$seeds, nb2listw(w5)) # vecinos, I= 0.6482
...
moran.test(CETE.fit$seeds,mat2listw(plots.dists.inv)) # 0.339
moran.test(CETE.fit$seeds, nb2listw(w5)) # vecinos, I= 0.4327
...
moran.test(SPRU.fit$seeds,mat2listw(plots.dists.inv)) # 0.5045
moran.test(SPRU.fit$seeds, nb2listw(w5)) # vecinos, I= 0.6252
...
moran.test(SOAS.fit$seeds,mat2listw(plots.dists.inv)) #  0.3722
moran.test(SOAS.fit$seeds, nb2listw(w5)) # vecinos, I=  0.4457
...
moran.test(BEMA.fit$seeds,mat2listw(plots.dists.inv)) #  0.4067
moran.test(BEMA.fit$seeds, nb2listw(w5)) # vecinos, I=  0.56028
...
moran.test(SCLA.fit$seeds,mat2listw(plots.dists.inv)) # 0.3790
moran.test(SCLA.fit$seeds, nb2listw(w5)) # vecinos, I= 0.4851
...
chmi.corr.s <- spline.correlog(x=CHMI.fit$x_coor2, y=CHMI.fit$y_coor2,
                               z=CHMI.fit$seeds, resamp=100, quiet=TRUE) #no works ----
plot(chmi.corr.s, main= " Spatial Autocorrelation CHMI fitness across plots")

moran.test(CHMI.fit$seeds,mat2listw(plots.dists.inv)) # 0.13
moran.test(CHMI.fit$seeds, nb2listw(w5)) # vecinos, I= 0.24
...
moran.test(juntas.plantas.fit$seeds,mat2listw(plots.dists.inv)) # I= 0.6711
moran.test(juntas.plantas.fit$seeds, nb2listw(w5)) # vecinos, I= 0.7596
...
plot(cete.corr.s, main= " Spatial Autocorrelation CETE fitness across plots",xlab="Distance (m)", ylab="Correlation")
plot(bema.corr.s, main= " Spatial Autocorrelation BEMA fitness across plots",xlab="Distance (m)", ylab="Correlation")
...
none.visitor<- none.visitor[,c("plot","subplot","Plant","Group","visits", "individuals", "seed","log.seed", "fruit", "visitas_indv", "visitas_indv_hora", "neigh_inter.plot","neigh_intra.plot",
                  "neigh_intra.3m", "neigh_inter.3m", "neigh_inter.1m", "neigh_intra.1m" , "neigh_inter.7.5" , "neigh_intra.7.5")] 
...
data <- data %>% group_by(plot, subplot, Plant, Group, individuals, seed,log.seed,fruit,visitas_indv_hora, x_coor2, y_coor2,
...
data <- data[,c("plot","subplot","Plant","Group","visits.total", "individuals", "seed","log.seed", "fruit",  "neigh_inter.plot","neigh_intra.plot",
...
ggsave(filename = paste("Graphics/CHFU.fitness.pdf",sep=""),
...
m.prueba.lema.2 <- lme(log.seed ~ Beetle+Fly+ Bee +neigh_inter.1m+neigh_intra.1m, data= LEMA.vis, random = ~1 |plot, control=lCtr,
                        method = "ML")# This model includes the log.seed
...
residplot(m.prueba.lema.2)#no se si se ajunta bien, no parece----
LEMA.vis$fittedvalues <- fitted(m.prueba.lema.2)
b <- ggplot(LEMA.vis, aes(x = neigh_inter.1m))+
    geom_point(aes(y= log.seed))+
    geom_smooth(method = "lm",aes(y=fittedvalues),family = "symmetric")+
...
#random structure
p1<- lme(log.seed ~ 1, data= PUPA.vis,random = ~1 |plot, control=lCtr,
         method = "ML")
p2 <- lme(log.seed ~ 1, data= PUPA.vis, random = ~1 |plot, control=lCtr,
          corr = corSpatial(form = ~x_coor2 + y_coor2, type ="gaussian", nugget = T), method = "ML") 
p3<- lme(log.seed ~ 1, data= PUPA.vis, random = ~1 |plot, control=lCtr,
         corr = corSpatial(form = ~x_coor2 + y_coor2, type ="rational", nugget = T), method = "ML")
p4 <- lme(log.seed ~ 1, data= PUPA.vis, random = ~1 |plot, control=lCtr,
          corr = corSpatial(form = ~x_coor2 + y_coor2, type ="exponential", nugget = T), method = "ML")
...
summary(model.avg(fmList.prueba.pupa.2))# nothing
...
#>MESU
MESU.vis <- subset(data.spread.visitors, Plant == "MESU") #solo 1 row
#>cete
CETE.vis <- subset(data.spread.visitors, Plant == "CETE") #solo 6 row
#>spru
...
data.spread.visitors$log.fly<- log((data.spread.visitors$Fly)+0.1)
data.spread.visitors$log.beetle<- log((data.spread.visitors$Beetle)+0.1)
data.spread.visitors$log.butterfly<- log((data.spread.visitors$Butterfly)+0.1)
data.spread.visitors$log.bee<- log((data.spread.visitors$Bee)+0.1)
...
#random structure
c.vis.bet.1<- lme(log(visits) ~ 1, data= Bet.vis,random = ~1 |plot, control=lCtr,
                method = "ML")
c2.vis.bet.1 <- lme(log(visits) ~ 1, data= Bet.vis, random = ~1 |plot, control=lCtr,
                  corr = corSpatial(form = ~x_coor2 + y_coor2, type ="gaussian", nugget = T), method = "ML")
c3.vis.bet.1<- lme(log(visits) ~ 1, data= Bet.vis, random = ~1 |plot, control=lCtr,
                 corr = corSpatial(form = ~x_coor2 + y_coor2, type ="rational", nugget = T), method = "ML")
c4.vis.bet.1 <- lme(log(visits) ~ 1, data= Bet.vis, random = ~1 |plot, control=lCtr,
                  corr = corSpatial(form = ~x_coor2 + y_coor2, type ="exponential", nugget = T), method = "ML")
...
#random structure
c.vis.fly.1<- lme(log(visits) ~ 1, data= fly.vis,random = ~1 |plot, 
                  method = "ML")
c2.vis.fly.1 <- lme(log(visits) ~ 1, data= fly.vis, random = ~1 |plot, 
                    corr = corSpatial(form = ~x_coor2 + y_coor2, type ="gaussian", nugget = T), method = "ML")
c3.vis.fly.1<- lme(log(visits) ~ 1, data= fly.vis, random = ~1 |plot, control=lCtr,
                   corr = corSpatial(form = ~x_coor2 + y_coor2, type ="rational", nugget = T), method = "ML")
c4.vis.fly.1 <- lme(log(visits) ~ 1, data= fly.vis, random = ~1 |plot, 
                    corr = corSpatial(form = ~x_coor2 + y_coor2, type ="exponential", nugget = T), method = "ML")
                    #the control of the model is the one that do the model to not work in that case. 
AIC(c.vis.fly.1, c2.vis.fly.1, c3.vis.fly.1, c4.vis.fly.1)# anyone with spatial autocor. 
...
#random structure
c.vis.bee.1<- lme(log(visits) ~ 1, data= bee.vis,random = ~1 |plot, control=lCtr,
                  method = "ML")
c2.vis.bee.1 <- lme(log(visits) ~ 1, data= bee.vis, random = ~1 |plot, control=lCtr,
                    corr = corSpatial(form = ~x_coor2 + y_coor2, type ="gaussian", nugget = T), method = "ML")
c3.vis.bee.1<- lme(log(visits) ~ 1, data= bee.vis, random = ~1 |plot, control=lCtr,
                   corr = corSpatial(form = ~x_coor2 + y_coor2, type ="rational", nugget = T), method = "ML")
c4.vis.bee.1 <- lme(log(visits) ~ 1, data= bee.vis, random = ~1 |plot, control=lCtr,
                    corr = corSpatial(form = ~x_coor2 + y_coor2, type ="exponential", nugget = T), method = "ML")
