#estos son ya los datos reales ---> LO HARE CON ABUNDANCIAS
library(tidyverse)
FV_19 <- subset(V, Year == 2019) 
#ver las spp de plantas frente el num de visitas
testB <- FV_19 %>% group_by(Plant_Simple, Group) %>% summarise(num.visits = sum(Visits))
pollinator.plot.2019 <- ggplot(testB) + 
    geom_boxplot(aes(x = Plant_Simple, y = num.visits)) +
    ggtitle("Number of visits per Plant")
    # facet_grid(plant~.)+
    NULL
pollinator.plot.2019
...
par(mfrow=c(2,1))
test19 <- FV_19 %>% group_by(Plot, Group_Floralvisitor) %>% summarise (num.pol = n())#con n() asi estoy contando cada una de las entradas que tengo
...
ab.19.1 <- Abun_2019 %>% group_by (Sp.Focal.Simpe, plot, subplot) %>% summarise(num.indv = sum (Plantas))
ab.19.1$Plot <- ab.19.1$plot
ab.19.1$Subplot <- ab.19.1$subplot
ab.19.1$Plant_Simple<- ab.19.1$Sp.Focal.Simpe

test.order.visits.16 <- FV_16 %>% group_by(Plot, Subplot, Visits, Plant, Group_Floralvisitor) %>% summarise(num.visits= sum(Visits))
test.order.visits.19 <- FV_19 %>% group_by(Plot, Subplot, Visits, Plant_Simple, Group_Floralvisitor) %>% summarise(num.visits= sum(Visits))
tabla.completa.16 <- dplyr::left_join (test.order.visits.16, ab.16)
tabla.completa.19.1 <- dplyr::left_join(test.order.visits.19, ab.19.1 )
...
boxplot(tabla.completa.19$num.indv~tabla.completa.19$Sp.Focal.Simpe, xlab= 'Species', ylab ='num.indv', main= "Num de indv por spp 2019")
...
x_scale1 <- scale_x_continuous(limits = c(0,80))
visitas.19 <- ggplot(tabla.completa.19, aes(x = num.indv, y = num.visits, group = Group_Floralvisitor))+
    geom_point(aes(color = Group_Floralvisitor))+
    geom_smooth(method = "lm", aes(color = Group_Floralvisitor))+
    x_scale1+
    ggtitle("visitas de los polinizadores segun las abundancias de plantas 2019")
    NULL
visitas.19
...
datos19 <- subset(tabla.completa.19, Plant %in% c("LEMA","CHFU","PUPA", "ME") & Group_Floralvisitor %in% c("Coleoptera", "Diptera", "Hemiptera", "Hymenoptera", "Lepidoptera", "Neuroptera"))
...
visitas.19.incomp <- ggplot(datos19, aes(x = num.indv, y = num.visits, group = Group_Floralvisitor))+
  ...
  ggtitle("visitas de los polinizadores segun la abundancia de las plantas en 2019 (seleccion de visitors)")
   NULL
visitas.19.incomp
...
tabla.MEPO.16 <- subset(tabla.completa.16, Plant == 'ME' & Group_Floralvisitor %in% c("Coleoptera", "Hymenoptera", "Diptera", "Lepidoptera", "Hemiptera"))
# no entiendo porque pero no funciona -->tabla.ME.19 <- subset(tabla.completa.19, Plant == 'ME' & Group_Floralvisitor %in% c("Coleoptera", "Hymenoptera", "Diptera", "Lepidoptera", "Hemiptera"))
...
visitas.MEPO.16 <- ggplot(tabla.MEPO.16, aes(x = num.indv, y = num.visits, group = Group_Floralvisitor))+
  ...
  NULL
visitas.MEPO.16
visitas.ME.19 <- ggplot(tabla.ME.19, aes(x = num.indv, y = num.visits, group = Group_Floralvisitor))+
  geom_point(aes(color = Group_Floralvisitor))+
  geom_smooth(method = "lm", aes(color = Group_Floralvisitor))+
  ylab("num.visits polinizadores")+
  xlab("abundancia MEPO")+
...
visitas.planta.19 <- ggplot(tabla.completa.19, aes(x = num.indv, y = num.visits, group = Plant_Simple))+
    geom_point(aes(color = Plant_Simple))+
    geom_smooth(method = "lm", aes(color = Plant_Simple))+
    x_scale2+
  ggtitle("numero de visitas en cada planta segun abundancia")
    NULL
visitas.planta.19
...
visitas.planta.incompleta19 <- ggplot(datos19, aes(x = num.indv, y = num.visits))+
  ...
  geom_smooth(method = "lm", aes(color = Group_Floralvisitor))+
  facet_wrap(Plant~Group_Floralvisitor)+
  ggtitle("como afecta cada floral visitor a cada una de las plantas")
  NULL
