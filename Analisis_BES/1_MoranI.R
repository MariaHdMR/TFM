...
data <- as.data.frame(data) #Maria, por que usas tidyverse para cargar datos, si luego los pasa a base r? M: Porque me daba problemas al cargarlos en base r por los puntos y comas
head(data)
...
moran.test(beetle.v$visits,mat2listw(plots.dists.inv)) # new: I= 0.1852 , p-value < 2.2e-16
moran.test(beetle.v$visits, nb2listw(w5)) # vecinos, new: I= 0.1271. Just curiosity, not for the paper
...
moran.test(flies.v$visits,mat2listw(plots.dists.inv)) # old: I= 0.0698 , new= 0.075 p-value = 3.851e-12
moran.test(flies.v$visits, nb2listw(w5)) # vecinos, old: I= 0.11, new: 0.066. Just curiosity, not for the paper
moran.plot(flies.v$visits,mat2listw(plots.dists.inv), main= "Spatial autocorrelation flies across plots")
moran.mc(flies.v$visits, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.01. statistic: 0.0754
...
moran.test(but.v$visits,mat2listw(plots.dists.inv)) # old: I= -1.032727e-02, homogenius; new: -0.0114, pvalue=0.7866
moran.test(but.v$visits, nb2listw(w5)) # vecinos, old: I=  -0.0125584112, new: -0.0135 . Just curiosity, not for the paper
moran.plot(but.v$visits,mat2listw(plots.dists.inv),  main= "Spatial autocorrelation butterflies across plots")
moran.mc(but.v$visits, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.95. statistic: -0.01148
...
moran.test(bee.v$visits,mat2listw(plots.dists.inv))# old: I = 0.03588, new: I= 2.678018e-02
moran.test(bee.v$visits, nb2listw(w5))# I= 0.0617 . Just curiosity, not for the paper
moran.plot(bee.v$visits,mat2listw(plots.dists.inv), main= "Spatial autocorrelation bees across plots")
moran.mc(bee.v$visits, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.02, statistic= 0.02678
...
moran.test(junto$visit, nb2listw(w5)) # vecinos, I= 0.2493 .Just curiosity, not for the paper
...
#                       and per plot
#IB: I don't think this is necessary for the paper. M: Okey. I silence that part, in the paper it is not included. 
...
moran.test(CHFU.p$num.planta,mat2listw(plots.dists.inv)) # I= 0.20. it is equall the new and the old. No differences
moran.test(CHFU.p$num.planta, nb2listw(w5)) # vecinos, I= 0.3198 . Just for curiosity, not included in the paper
moran.plot(CHFU.p$num.planta,mat2listw(plots.dists.inv), main= " Spatial Autocorrelation CHFU across plots")
moran.mc(CHFU.p$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.01, statistic= 0.20
...
moran.test(LEMA.2$num.planta,mat2listw(plots.dists.inv)) # I= 0.24. the same the new and the old
moran.test(LEMA.2$num.planta, nb2listw(w5)) # vecinos, I= 0.39 . Just for curiosity, not included in the paper
moran.plot(LEMA.2$num.planta,mat2listw(plots.dists.inv), main= " Spatial Autocorrelation LEMA across plots")
moran.mc(LEMA.2$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.01, statistic= 0.24
...
moran.test(PUPA.p1$num.planta,mat2listw(plots.dists.inv)) # I= 0.2128. the same the new and the old
moran.test(PUPA.p1$num.planta, nb2listw(w5)) # vecinos, I= 0.288 . Just for curiosity, not included in the paper
moran.plot(PUPA.p1$num.planta,mat2listw(plots.dists.inv), main= " Spatial Autocorrelation PUPA across plots")
moran.mc(PUPA.p1$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.01, statistic= 0.2129
...
moran.test(MESU.p$num.planta,mat2listw(plots.dists.inv)) # I= 0.153. the same the new and the old
moran.test(MESU.p$num.planta, nb2listw(w5)) # vecinos, I= 0.20 .Just for curiosity, not included in the paper
moran.plot(MESU.p$num.planta,mat2listw(plots.dists.inv), main= " Spatial Autocorrelation MESU across plots")
moran.mc(MESU.p$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.01, statistic= 0.153
...
moran.test(CHMI.p$num.planta,mat2listw(plots.dists.inv)) # I= 2.779055e-02 . the same the new and the old
...
moran.mc(CHMI.p$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.01. statistic= 0.027
...
moran.test(CETE.p$num.planta,mat2listw(plots.dists.inv)) # I= 0.1435. the same the new and the old
...
moran.mc(CETE.p$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.01. statistic= 0.14351
...
moran.test(BEMA.p$num.planta,mat2listw(plots.dists.inv)) # I=  0.1452. the same the new and the old
...
moran.mc(BEMA.p$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value= 0.01. statistic= 0.1452
...
moran.test(SCLA.p$num.planta,mat2listw(plots.dists.inv)) # I= 0.119. the same the new and the old
...
moran.mc(SCLA.p$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01. statistic= 0.119
...
moran.test(SOAS.p$num.planta,mat2listw(plots.dists.inv)) # I= 0.2331. the same the new and the old
...
moran.mc(SOAS.p$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01. statistic= 0.2332
...
moran.test(SPRU.p$num.planta,mat2listw(plots.dists.inv)) # I= 0.1086. the same the new and the old
...
moran.mc(SPRU.p$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01. statistic= 0.1087

#plantas juntas --> not included in the paper. 
...
moran.test(num.plant$num.planta,mat2listw(plots.dists.inv)) # I= 0.21. the new and the old the same
...
moran.mc(num.plant$num.planta, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01. statistic= 0.21
...
moran.test(CHFU.fit$seeds,mat2listw(plots.dists.inv)) # old: I= 0.1032, new: 0.0569
moran.test(CHFU.fit$seeds, nb2listw(w5)) # vecinos, I= 0.1921, new: 0.164
moran.plot(CHFU.fit$seeds,mat2listw(plots.dists.inv), main= " Spatial Autocorrelation CHFU fitness (seeds) across plots")
moran.mc(CHFU.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, statistic new: 0.0569
...
moran.test(LEMA.fit$seeds,mat2listw(plots.dists.inv)) # I= 0.1065, new: 0.1577
...
moran.mc(LEMA.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, new statistic: 0.1577
#IB: Same here
...
moran.test(PUPA.fit$seeds,mat2listw(plots.dists.inv)) # 0.058, new I= 3.433128e-02
...
moran.mc(PUPA.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, new statistic= 0.03433
...
moran.test(MESU.fit$seeds,mat2listw(plots.dists.inv)) # 0.27, new I= 0.158846
...
moran.mc(MESU.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, new statistic= 0.1588
...
moran.test(CETE.fit$seeds,mat2listw(plots.dists.inv)) # 0.118, new= 0.1231
...
moran.mc(CETE.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, new statistic= 0.12341
...
moran.test(SPRU.fit$seeds,mat2listw(plots.dists.inv)) # 0.2177, new= 0.1534
...
moran.mc(SPRU.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01. new statistic= 0.15344
...
moran.test(SOAS.fit$seeds,mat2listw(plots.dists.inv)) #  0.1463, new= 0.1355
...
moran.mc(SOAS.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, new statistic= 0.13553
...
moran.test(BEMA.fit$seeds,mat2listw(plots.dists.inv)) #  0.11, new= 0.114
...
moran.mc(BEMA.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, new statistic= 0.1149
...
moran.test(SCLA.fit$seeds,mat2listw(plots.dists.inv)) # 0.1643, new= 0.1404
...
moran.mc(SCLA.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, new statistic= 0.14048
...
moran.test(CHMI.fit$seeds,mat2listw(plots.dists.inv)) # 0.0556, new= 1.136502e-02
...
moran.mc(CHMI.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, new statistic= 0.011365
...
moran.test(juntas.plantas.fit$seeds,mat2listw(plots.dists.inv)) # I= 0.1510, new= 0.1463
...
moran.mc(juntas.plantas.fit$seeds, mat2listw(plots.dists.inv), nsim=99) #p-value=0.01, new statistic= 0.14633
...
#Now with the data of the seeds per individual = (seeds/one fruit)*fruits----
#I decided to only included in the paper in this part, the reproductive sucess as numer of seeds per one fruit. So I silence that part
#CHFU 
