...
fmList.prueba.chufu.2.fr <- get.models(m.prueba_sec.chufu.2.fr, 1:4) 
...
c4.vis.bet.1 <- lme(visits ~ 1, data= Bet.vis, random = ~ 1|plot/Plant, control=lCtr,
                    corr = corSpatial(form = ~x_coor2 + y_coor2, type ="exponential", nugget = T), method = "ML")
AIC(c.vis.bet.1, c2.vis.bet.1, c3.vis.bet.1, c4.vis.bet.1)#best model 3
...
Bet.vis1$fittedvalues <- fitted(k)
Bet.plot <- ggplot(Bet.vis1, aes(x = flowers2))+
...
AIC(c.vis.bee.1, c2.vis.bee.1, c3.vis.bee.1, c4.vis.bee.1)#the 2nd model is the best.

bee.vis1 <- bee.vis[!is.na(bee.vis$flowers2),]

k2.bee <- lme(log(visits) ~ neigh_inter.1m+ neigh_intra.1m+flowers2, data= bee.vis1,random = ~1|plot/Plant, control=lCtr,
              corr = corSpatial(form = ~x_coor2 + y_coor2, type ="gaussian", nugget = T), method = "ML")
...
AIC(c.vis.fly.1, c2.vis.fly.1, c3.vis.fly.1, c4.vis.fly.1)#the 2nd model is the best.
